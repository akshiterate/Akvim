...
int editorCxToRx(erow& row, int cx){
	int rx =0;
	for(int i=0;i<cx;i++){
		if(row.chars[i] == '\t') rx += (AKVIM_TAB_STOP-1)-(rx%AKVIM_TAB_STOP);
		rx++;
	}
	return rx;
}
...
void editorUpdateRow(erow& row){
	...
	int idx = 0;
	row.render.resize(row.chars.length() + tabs*(AKVIM_TAB_STOP-1) +1);
	for(int i=0;i<row.chars.length();i++){
		if (row.chars[i] == '\t'){
			row.render[idx++] = ' ';
			while(idx%AKVIM_TAB_STOP !=0) {
				row.render[idx++] = ' ';
			}
		}
		else{
...
void editorRowInsertChar(erow *row, int at, int c){
	if(at <0 || at>(int)row->chars.size()) at = row->chars.size();
